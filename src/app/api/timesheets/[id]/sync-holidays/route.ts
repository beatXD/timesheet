...
// POST /api/timesheets/[id]/sync-holidays - Sync holidays into existing timesheet
export async function POST(
  ...
  try {
    ...
    // Only allow sync when timesheet is in draft or rejected status
    if (!["draft", "rejected"].includes(timesheet.status)) {
      return NextResponse.json(
        { error: "Can only sync holidays for draft or rejected timesheets" },
        { status: 400 }
      );
    }
